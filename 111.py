...
def start(message):
	...
	b1 = types.KeyboardButton("Отправить фото на проверку")
	markup.add(b1)
...
def message(message):
	...
	if message.text == "Отправить фото на проверку":
		bot.send_message(message.from_user.id, "Хорошо жду") 
...
def handle_photo(message):
	...
	bot.send_message(message.from_user.id, "Идёт обработка") 
	def object1():
